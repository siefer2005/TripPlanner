...
type CafeRouteProp = RouteProp<RootStackParamList, 'Cafe'>;
...
const CafeScreen = () => {
    ...
    const route = useRoute<CafeRouteProp>();
    const { location } = route.params || { location: 'India' };
    ...
    const fetchCafes = async () => {
        setLoading(true);
        try {
            const query = `top rated cafes in ${location}`;
            ...
            if (data.results) {
                const sortedDetails = data.results.sort((a: Cafe, b: Cafe) => (b.rating || 0) - (a.rating || 0));
                setCafes(sortedDetails);
            }
        } catch (error) {
        ...
        <SafeAreaView style={styles.container}>
            ...
            {loading ? (
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={{ marginTop: 10, color: '#666' }}>Finding best places for coffee...</Text>
                </View>
            ) : (
                <FlatList
                    data={cafes}
                    ...
                    ListEmptyComponent={
                        <View style={styles.centerEmpty}>
                            <Text style={{ fontSize: 16, color: 'gray' }}>No cafes found.</Text>
                        </View>
                    }
                />
            )}
